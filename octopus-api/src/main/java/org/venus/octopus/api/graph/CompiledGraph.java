 ...
 * 已编译图接口
 * <p>
 * 通过 {@link GraphBuilder#compile()} 生成，持有图的完整结构定义和运行器。
 * 提供两种执行模式：
 * <ul>
...
public interface CompiledGraph<S extends AgentState> {
     ...
     * @return 图执行完毕后的最终状态
     * @throws org.venus.octopus.common.exception.GraphException 若执行过程中发生错误
     */
    S invoke(S initialState);

    /**
     * 流式执行图，逐节点返回中间状态
     * <p>每个 Stream 元素代表执行完一个节点后的状态快照。</p>
     *
     * @param initialState 初始状态
     * @return 节点状态流
     */
